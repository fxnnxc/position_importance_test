...
class Rule1(BaseRule):
    ...
    def make_sample(self, i:int):
        ...
        m = np.random.randint(1, self.num_shapes)
        c = (a+b)% m
        assert c < self.num_shapes        
        sample[1, 0] = a 
        sample[2, 0] = self.add_token
        sample[3, 0] = b
        sample[4, 0] = self.mod_equal_token
        sample[5, 0] = m
        sample[6, 0] = c
        return sample 
...
class Rule2(BaseRule):
    ...
    def make_sample(self, i:int):
        sample = torch.zeros(self.max_seq_len, 3).fill_(self.eos_token)
        a = np.random.randint(1, self.num_shapes-1)
        b = np.random.randint(1, max(self.num_shapes//(a),2) )
        c = a*b
        assert c < self.num_shapes        
        sample[1, 0] = a 
        sample[2, 0] = self.multiply_token
        sample[3, 0] = b
        sample[4, 0] = self.equal_token
        sample[5, 0] = c
        return sample
...
class Rule5(BaseRule):
    ...
    def make_sample(self, i:int):
        ...
        if c >= 0:
            ...
            sample[6, 0] = self.negative_token
            sample[7, 0] = -c
        return sample
...
class Rule6(BaseRule):
    ...
    def make_sample(self, i:int):
        sample = torch.zeros(self.max_seq_len, 3).fill_(self.eos_token)
        a = np.random.randint(2, self.num_shapes-1)
        b = np.random.randint(1, max(2,a))
        c = a//b
        assert c < self.num_shapes        
        sample[1, 0] = a 
        sample[2, 0] = self.divide_token
        sample[3, 0] = b
        sample[4, 0] = self.equal_token
        sample[5, 0] = c
        return sample
...
class Rule7(BaseRule):
    ...
    def make_sample(self, i:int):
        sample = torch.zeros(self.max_seq_len, 3).fill_(self.eos_token)
        a = np.random.randint(2, self.num_shapes)
        b = np.random.randint(1, a)
        m = np.random.randint(1, max(a//b, 2))
        c = (a//b)% m
        assert c < self.num_shapes        
        sample[1, 0] = a 
        sample[2, 0] = self.divide_token
